  ...
  try {
    ...
    const totals = db.prepare(`
      SELECT 
        ...
        COALESCE(SUM(total_price),0) gross_revenue,
        COALESCE(SUM(tip),0) total_tips
      FROM orders 
    ...
    const calc = (rows) =>
      rows.map(r => ({
        ...r,
        rider_earnings: Math.round((r.gross || 0) * rate),
        platform_fee: Math.round((r.gross || 0) * (1 - rate))
      }));

    res.json({
      profile,
      daily: calc(db.prepare(`
        SELECT date(created_at) day,
        COUNT(*) deliveries,
        COALESCE(SUM(total_price),0) gross,
        COALESCE(SUM(tip),0) tips
        FROM orders
        WHERE rider_id=? AND status='delivered'
        GROUP BY day
        ORDER BY day DESC
      ...
      totals: {
        ...totals,
        rider_earnings: Math.round(totals.gross_revenue * rate),
        platform_fee: Math.round(totals.gross_revenue * (1 - rate))
      },

      payouts: db.prepare(
        'SELECT * FROM payouts WHERE rider_id=? ORDER BY created_at DESC LIMIT 20'
      ).all(req.user.id),
    ...
    console.error(e);
    res.status(500).json({ error: 'Earnings failed' });
  }
});
...
router.post('/withdrawal', rider, async (req, res) => {
  try {
    ...
    if (!amount || amount < 500)
      return res.status(400).json({ error: 'Minimum MWK 500' });

    const id = uuidv4();

    db.prepare(`
      INSERT INTO payouts(id,rider_id,amount,notes)
      VALUES(?,?,?,?)
    `).run(
      id,
      req.user.id,
      amount,
      `Withdrawal to ${phone_number || req.user.phone}`
    );
    ...
    await sendSMS(
      req.user.phone,
      `Withdrawal request received: MWK ${amount}`
    ).catch(() => {});
...
router.post('/orders/:orderId/proof', rider, upload.single('photo'), (req, res) => {
  try {
    ...
    db.prepare(
      "UPDATE orders SET proof_photo=? WHERE id=?"
    ).run(photo, order.id);
